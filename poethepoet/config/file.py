...
Always keep the pyproject if it's there
- decouple task sfrom pyproject


If we find poe_tasks.yaml exists in a subdir of a project, we don't want to have to
    search parent directories for pyproject.toml
    - Make sure executer config can reference a project dir for the poetry executor??
If we find pyproject.toml with tool.poe, we stop searching
If we hit a pyproject.toml before the poe_tasks file then keep it!
